...
from shapes_2d import Shapes2D
from slate import SLATE


# Start a wandb run with `sync_tensorboard=True`
wandb.init(project='slate')
...
parser.add_argument('--data_path', default='../c-swm/data/shapes_train.h5')
...
parser.add_argument('--num_slots', type=int, default=10)  # interesting
parser.add_argument('--num_slot_heads', type=int, default=1)  # interesting
parser.add_argument('--slot_size', type=int, default=16)  # interesting
...
parser.add_argument('--dataset', type=str, default='shapes2D')
...
log_dir = os.path.join(args.log_path, str(int(round(datetime.today().timestamp()))))
os.makedirs(log_dir, exist_ok=True)
wandb.config.update({k: v for k, v in vars(args).items()})

if args.dataset == 'shapes2D':
    dataset = Shapes2D
...
train_dataset = dataset(root=args.data_path, phase='train')
val_dataset = dataset(root=args.data_path, phase='val')
...
for epoch in range(start_epoch, args.epochs):
    ...
    with torch.no_grad():
        ...
        wandb.log({'TRAIN/recon': wandb.Image(grid)}, global_step)
    ...
    with torch.no_grad():
        ...
        wandb.log({'VAL/loss_relax': val_loss_relax,
        ...
        if val_loss < best_val_loss:
            ...
            if 50 <= epoch:
                ...
                vis_recon = visualize(image, recon, gen_img, attns, N=32)
                grid = vutils.make_grid(vis_recon, nrow=args.num_slots + 3, pad_value=0.2)[:, 2:-2, 2:-2]
                wandb.log({'VAL/recon': wandb.Image(grid)}, global_step)
        ...
        wandb.log({'VAL/best_loss': best_val_loss}, global_step)
        ...
        chkpt = wandb.Artifact('slate', type='model')
        chkpt.add_file(os.path.join(log_dir, 'checkpoint.pt.tar'))  # Adds multiple files to artifact
        wandb.log_artifact(chkpt)
