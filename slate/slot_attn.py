...
class SlotAttentionEncoder(nn.Module):
    ...
    def __init__(self, num_iterations, num_slots,
                 input_channels, slot_size, mlp_hidden_size, pos_channels, num_heads, mode='classic'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        elif self.mode == 'same_init':
            slots = x.new_empty(self.num_slots, self.slot_size).normal_().unsqueeze(0).repeat(B, 1, 1)
            slots = self.slot_mu + torch.exp(self.slot_log_sigma) * slots
        elif self.mode == 'first':
            slots = x.new_empty(1, self.num_slots, self.slot_size).normal_()
            slots = self.slot_mu + torch.exp(self.slot_log_sigma) * slots
            slots, _ = self.slot_attention(x[:1], slots)
            slots = slots.unsqueeze(0).repeat(B, 1, 1)
        else:
